package example.fussen.watch.widget;
...
import example.fussen.watch.utils.Utils;
...
public class WatchView extends View {
    ...
    private void obtainStyledAttrs(AttributeSet attrs) {
        ...
        try {
            ...
            mPadding = array.getDimension(R.styleable.WatchView_padding, DptoPx(10));
            mTextSize = array.getDimension(R.styleable.WatchView_text_size, SptoPx(16));
            mHourPointWidth = array.getDimension(R.styleable.WatchView_hour_pointer_width, DptoPx(5));
            mMinutePointWidth = array.getDimension(R.styleable.WatchView_minute_pointer_width, DptoPx(3));
            mSecondPointWidth = array.getDimension(R.styleable.WatchView_second_pointer_width, DptoPx(2));
            mPointRadius = (int) array.getDimension(R.styleable.WatchView_pointer_corner_radius, DptoPx(10));
            mPointEndLength = array.getDimension(R.styleable.WatchView_pointer_end_length, DptoPx(10));

            mColorLong = array.getColor(R.styleable.WatchView_scale_long_color, Color.argb(225, 0, 0, 0));
            mColorShort = array.getColor(R.styleable.WatchView_scale_short_color, Color.argb(125, 0, 0, 0));
            mHourPointColor = array.getColor(R.styleable.WatchView_hour_pointer_color, Color.BLACK);
            mMinutePointColor = array.getColor(R.styleable.WatchView_minute_pointer_color, Color.BLACK);
            mSecondPointColor = array.getColor(R.styleable.WatchView_second_pointer_color, Color.RED);
        } catch (Exception e) {
            //一旦出现错误全部使用默认值
            mPadding = DptoPx(10);
            mTextSize = SptoPx(16);
    ...
    private float DptoPx(int value) {
        ...
        return Utils.Dp2Px(getContext(), value);
    }

    //sp转px
    private float SptoPx(int value) {
        return Utils.Sp2Px(getContext(), value);
    }
    ...
    private void paintPointer(Canvas canvas) {
        ...
        int angleHour = (int) ((hour % 12) * 360 / 12 + minute * 0.5); //时针转过的角度
    ...
    private void paintScale(Canvas canvas) {
        ...
        for (int i = 0; i < 60; i++) {
            if (i % 5 == 0) { //整点
                mPaint.setStrokeWidth(Utils.Dp2Px(getContext(), 1.5f));
